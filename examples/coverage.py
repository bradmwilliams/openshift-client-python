...
try:
    ...
    oc.selector("projects").narrow(
        ...
        lambda prj: prj.metadata.qname == "openshift"
        or prj.metadata.qname.startswith("openshift-")
        or prj.metadata.qname == "kubernetes"
        or prj.metadata.qname.startswith("kube-")
        or prj.metadata.qname.startswith("kubernetes-")
    ).for_each(
    ...
    with oc.timeout(5):
        success, obj = (
            ...
            .narrow("pod")
            .until_any(
                lambda pod: pod.status.conditions.can_match(
    ...
    with oc.project("myproject") as project:
        ...
        assert label_selector.qnames()[0] == "users/john"

        users.label({"another_label": "another_value"})

        john.object().patch(
            {"groups": null, "identities": ["github: 19783215"]},
        )
