...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-k", "--kubeconfig", help="The kubeconfig to create", required=True
    )
    parser.add_argument(
        "-s", "--server", help="The API Server to communicate with", required=True
    )
    ...
    with oc.timeout(60 * 30), oc.tracking() as t, my_context:
        if oc.get_config_context() is None:
            print(
                f"Current context not set! Logging into API server: {my_context.api_server}\n"
            )
            try:
                oc.invoke("login")
