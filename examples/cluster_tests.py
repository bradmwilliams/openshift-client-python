...
def simple_http_server_resources(
    name, port=8080, create_service=False, create_route=False
):
    """
    ...
    def create_test_project(suffix, port):
        ...
        with oc.new_project(project_name):
            ...
            server_sel = oc.create(
                simple_http_server_resources(
                    server_name, port, create_service=True, create_route=True
                )
            )
            ...
            server_sel.narrow("route").until_all(
                1, success_func=oc.status.is_route_admitted
            )

            # Create a passive pod that blocks forever so we exec commands within it
            client_sel = oc.create(
                oc.build_pod_simple(
                    client_name, image="python:3", command=["tail", "-f", "/dev/null"]
                )
            )
            ...
            report_progress(
                "Ensure client pod can communicate to server pod IP in same namespace"
            )
            client_pod.execute(
                cmd_to_exec=[
                    "curl",
                    "http://{}:{}".format(server_pod.model.status.podIP, port),
                ],
                auto_raise=True,
            )

            report_progress(
                "Ensure client pod can communicate to server service IP in same namespace"
            )
            client_pod.execute(
                cmd_to_exec=[
                    "curl",
                    "http://{}:{}".format(service.model.spec.clusterIP, port),
                ],
                auto_raise=True,
            )

            report_progress(
                "Ensure client pod can communicate to server service DNS in same namespace"
            )
            client_pod.execute(
                cmd_to_exec=["curl", "http://{}:{}".format(server_name, port)],
                auto_raise=True,
            )

            report_progress(
                "Ensure client pod can communicate to server route in same namespace"
            )
            client_pod.execute(
                cmd_to_exec=["curl", "http://{}".format(route.model.spec.host)],
                auto_raise=True,
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run a series of tests against a cluster"
    )
    parser.add_argument(
        "-b",
        "--bastion",
