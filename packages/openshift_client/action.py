...
token_regex = re.compile(
    r"[a-zA-Z0-9+/_\-]{10,}\.[a-zA-Z0-9+/_\-]{100,}\.[a-zA-Z0-9+/_\-]{20,}"
)
...
class Action(object):
    ...
    def as_json(
        self, indent=4, redact_tokens=True, redact_streams=True, redact_references=True
    ):
        return json.dumps(
            self.as_dict(
...
def oc_action(
    ...
    **kwargs
    ...
    if context.get_ssh_client() is not None:
        references[".client_host"] = "{}@{}".format(
            context.get_ssh_username() or "", context.get_ssh_hostname()
        )
        ...
        if context.get_ssh_client() is not None:
            ...
            try:
                ...
                (
                    ...
                    ssh_stdout,
                    ssh_stderr,
                ) = context.get_ssh_client().exec_command(
                    command=pathed_command,
                    timeout=remaining,
                    environment={
                        "LC_ALL": "en_US.UTF-8",
                    },
                ...
                with TempFile() as out:
                    with TempFile() as err:
                        ...
                        while process.poll() is None:
                            ...
                            period = min(
                                1, period + period
                            )  # Poll fast at first, but slow down to 1/sec over time
    ...
    if timeout and timeout_context and timeout_context.frame_info:
        references[".timeout_context"] = "{}:{}".format(
            timeout_context.frame_info[0], timeout_context.frame_info[1]
        )
