...
class TestSelector(unittest.TestCase):
    def test_qname_matches(self):
        ...
        self.assertTrue(
            qname_matches("template/django", ["template.template.openshift.io/django"])
        )
        self.assertFalse(
            qname_matches(
                "template/django", ["template.template.openshift.io/django.2"]
            )
        )
        self.assertTrue(qname_matches("template/django", ["template.template/django"]))
        self.assertFalse(
            qname_matches("template/django", ["template.template/django.2"])
        )

        self.assertFalse(
            qname_matches("template2/django", ["template.template.openshift.io/django"])
        )
        self.assertFalse(
            qname_matches("template/django2", ["template.template.openshift.io/django"])
        )
    ...
    def test_set_operations(self):
        ...
        t2 = selector(
            [
                "template.template.openshift.io/django",
                "template.template.openshift.io/django2",
            ]
        )
