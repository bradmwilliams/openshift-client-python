...
with oc.tracking() as t:
    with oc.client_host(
        hostname="18.222.71.125", username="root", auto_add_host=True
    ):  # free-stg
        with oc.project("openshift-monitoring"):
            try:

                result = (
                    oc.selector("pod/alertmanager-main-0")
                    .object()
                    .execute(
                        ["cat"], container_name="alertmanager", stdin="stdin for cat"
                    )
                )
                print(result.out())
                exit(0)
                ...
                oc.selector(
                    "node/pod_ip-172-31-79-85.us-east-2.compute.internal"
                ).object().patch(
                    {"metadata": {"annotations": {"cr_patch": "yes"}}},
                    strategy="strategic",
                    cmd_args=["-o=yaml"],
                )
